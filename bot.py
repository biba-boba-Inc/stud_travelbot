...
def send_text(message):
    ...
    if users[iden]["city"] and text == "Вывести список достопримечательностей":
        s = "Список достопримечательностей:\r\n"
        for row in cursor.execute('''SELECT name FROM Unit WHERE Unit.city_id = (SELECT City.id FROM City JOIN userinfo on City.name = userinfo.city)'''):
            s += row[0] + '\r\n'
        if len(s) ==  len("Список достопримечательностей:\r\n"):
            s = "В нашей базе данных еще нет достопримечательностей в выбранном городе"
